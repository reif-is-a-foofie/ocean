...
from .tools.design_system import DESIGN_METHOD, DESIGN_PRINCIPLES
...
class AgentBase:
    ...
    def introduce_detail(self) -> str:
        """Role + tools line without name prefix (for feed agent_say)."""
        s = self.role
        if self.tools:
            s += f" (tools: {', '.join(self.tools)})"
        return s
...
class Edna(AgentBase):
    def __init__(self) -> None:
        ...
        super().__init__(
            name="Edna",
            role="Designer/UI/UX. Ships interfaces (HTML/CSS, tokens, a11y); uses host screenshot tools for visual QA when available.",
            tools=tool_list,
        )

    def plan(self, spec: ProjectSpec) -> list[str]:
        """Provide design methodology based on project type"""
        principle_heads = [p.split(":", 1)[0].strip() for p in DESIGN_PRINCIPLES[:3]]
        base_principles = f"Principles checkpoint: {', '.join(principle_heads)}"
        if spec.kind == "cli":
            return [
            ...
            return [
                ...
                *DESIGN_METHOD[:4],
                base_principles,
                "Capture key screens for visual regression when tools allow",
            ]
        else:
            return [step.split(":", 1)[0] for step in DESIGN_METHOD[:3]] + [base_principles]
