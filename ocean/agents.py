...
class Moroni(AgentBase):
    ...
    def _generate_architecture(self, spec: ProjectSpec):
        ...
        if codex_exec.available():
            ...
            if verbose:
                print(f"🗣️ [Q] Context ready: {bundle}")
            prd_text = ""
            p = Path("docs/prd.md")
            if p.exists():
                try:
            ...
            if _v():
                ...
                print(f"🗣️ [Moroni] Codex exec request → {preview}")
            files = codex_exec.generate_files(instruction, ["docs/architecture.md"], bundle)
            if files:
                from .codex_exec import last_mode
                for rel, content in files.items():
                    Path(rel).parent.mkdir(parents=True, exist_ok=True)
                    Path(rel).write_text(content, encoding="utf-8")
                print(f"🤖 [Moroni] Generated architecture via Codex exec ({last_mode()})")
                return
        ...
        if files:
            ...
            print("🤖 [Moroni] Generated architecture via Codex MCP")
            return

        # Fallback: generate locally if MCP unavailable or returned nothing
        if spec.kind == "cli":
...
"""
            ...
            arch_content = f"""# {spec.name} - Web Architecture
...
"""
            # Also generate API design stub
            api_design = f"""# {spec.name} - API Design
...
Generated by OCEAN (local fallback)
"""
            (docs_dir / "api_design.md").write_text(api_design, encoding="utf-8")
        else:
            arch_content = f"""# {spec.name} - Architecture

## Overview
- Type: {spec.kind}
- Goals: {', '.join(spec.goals) or '(tbd)'}
- Constraints: {', '.join(spec.constraints) or '(none)'}
...
"""
        ...
        print("🤖 [Moroni] Generated architecture (local fallback)")
        return
...
'''
    ...
    def _generate_web_backend(self, spec: ProjectSpec):
        ...
        if codex_exec.available():
            ...
            if verbose:
                print(f"🗣️ [Q] Using context bundle: {bundle}")
            files = codex_exec.generate_files(instruction, ["backend/app.py"], bundle)
            if files:
                from .codex_exec import last_mode
                for rel, content in files.items():
                    Path(rel).parent.mkdir(parents=True, exist_ok=True)
                    Path(rel).write_text(content, encoding="utf-8")
                print(f"🤖 [Q] Generated web backend via Codex exec ({last_mode()})")
                return
        ...
        if files:
            for rel, content in files.items():
                MCP.write_file(self.name, Path(rel), content)
            print("🤖 [Q] Generated web backend via Codex MCP")
            return
...
'''
        ...
        print(f"🤖 [Q] Generated web backend (fallback)")
...
"""
    ...
    def _generate_web_design(self, spec: ProjectSpec):
        ...
        if codex_exec.available():
            ...
            files = codex_exec.generate_files(instruction, ["ui/index.html", "ui/styles.css"], bundle)
            if files:
                from .codex_exec import last_mode
                for rel, content in files.items():
                    Path(rel).parent.mkdir(parents=True, exist_ok=True)
                    Path(rel).write_text(content, encoding="utf-8")
                print(f"🤖 [Edna] Generated web interface via Codex exec ({last_mode()})")
                return
        ...
        if files:
            for rel, content in files.items():
                MCP.write_file(self.name, Path(rel), content)
            print("🤖 [Edna] Generated web interface via Codex MCP")
            return
...
footer {{
    ...
    opacity: 0.8;
        ...
        print(f"🤖 [Edna] Generated web interface (fallback)")
...
class Mario(AgentBase):
            ...
            for f in sorted(p.rglob('*')):
                if f.is_file() and f.suffix in {'.py', '.md', '.json', '.yml', '.yaml', '.html', '.css'}:
                    try:
                        ...
                        pass
        out.write_text('\n\n'.join(lines) + '\n', encoding='utf-8')
        print(f"🤖 [Mario] Prepared codegen context: {out}")
        return out
    ...
    def _generate_ci_workflow(self, spec: ProjectSpec):
        ...
        if codex_exec.available():
            ...
            files = codex_exec.generate_files(instruction, [".github/workflows/ci.yml"], bundle)
            if files:
                from .codex_exec import last_mode
                for rel, content in files.items():
                    Path(rel).parent.mkdir(parents=True, exist_ok=True)
                    Path(rel).write_text(content, encoding="utf-8")
                print(f"🤖 [Mario] Generated CI workflow via Codex exec ({last_mode()})")
                return
        ...
        if files:
            for rel, content in files.items():
                MCP.write_file(self.name, Path(rel), content)
            print(f"🤖 [Mario] Generated CI workflow via Codex MCP")
            return
...
'''
        ...
        print(f"🤖 [Mario] Generated CI workflow (fallback)")
    ...
    def _generate_docker_config(self, spec: ProjectSpec):
        ...
        if codex_exec.available():
            ...
            files = codex_exec.generate_files(instruction, ["Dockerfile", ".dockerignore"], bundle)
            if files:
                from .codex_exec import last_mode
                for rel, content in files.items():
                    Path(rel).parent.mkdir(parents=True, exist_ok=True)
                    Path(rel).write_text(content, encoding="utf-8")
                print(f"🤖 [Mario] Generated Docker config via Codex exec ({last_mode()})")
                return
        ...
        if files:
            for rel, content in files.items():
                MCP.write_file(self.name, Path(rel), content)
            print(f"🤖 [Mario] Generated Docker config via Codex MCP")
            return
...
'''
        ...
        print(f"🤖 [Mario] Generated Docker config (fallback)")
    ...
    def _generate_deployment_config(self, spec: ProjectSpec):
        ...
        if codex_exec.available():
            ...
            files = codex_exec.generate_files(instruction, ["devops/deploy.yaml"], bundle)
            if files:
                from .codex_exec import last_mode
                for rel, content in files.items():
                    Path(rel).parent.mkdir(parents=True, exist_ok=True)
                    Path(rel).write_text(content, encoding="utf-8")
                print(f"🤖 [Mario] Generated deployment config via Codex exec ({last_mode()})")
                return
        ...
        if files:
            for rel, content in files.items():
                MCP.write_file(self.name, Path(rel), content)
            print(f"🤖 [Mario] Generated deployment config via Codex MCP")
            return
...
'''
        ...
        print(f"🤖 [Mario] Generated deployment config (fallback)")
    ...
    def _start_local_runtime(self, spec: ProjectSpec) -> str | None:
        ...
        if (
            ...
            print("📦 [Mario] Installing runtime dependencies (fastapi[all], uvicorn)...")
            try:
                subprocess.run(
                    [
                        sys.executable,
                ...
                uvicorn_available = True
                print("✅ [Mario] Dependencies installed.")
            except subprocess.CalledProcessError as e:
                print(
                    f"❌ [Mario] Failed to install dependencies automatically: exit={e.returncode}. "
                    "Run 'ocean run --yes' or install manually."
                )
