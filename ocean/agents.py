...
class Moroni(AgentBase):
    ...
    def _generate_architecture(self, spec: ProjectSpec):
        ...
        if not files:
            raise RuntimeError("Codex MCP did not return architecture files (MCP-only mode)")
        for rel, content in files.items():
            MCP.write_file(self.name, Path(rel), content)
        print("🤖 [Moroni] Generated architecture via Codex MCP")
        return

        # dead code path retained for reference (MCP-only prevents fallback)
        if False and spec.kind == "cli":
...
Generated by OCEAN using Codex MCP
"""
            (docs_dir / "api_design.md").write_text(api_design)
        else:
...
"""
        ...
        print(f"🤖 [Moroni] Generated architecture (fallback)")
...
'''
    ...
    def _generate_web_backend(self, spec: ProjectSpec):
        ...
        if not files:
            raise RuntimeError("Codex MCP did not return backend files (MCP-only mode)")
        for rel, content in files.items():
            MCP.write_file(self.name, Path(rel), content)
...
"""
    ...
    def _generate_web_design(self, spec: ProjectSpec):
        ...
        if not files:
            raise RuntimeError("Codex MCP did not return UI files (MCP-only mode)")
        for rel, content in files.items():
            MCP.write_file(self.name, Path(rel), content)
...
class Mario(AgentBase):
    ...
    def _generate_ci_workflow(self, spec: ProjectSpec):
        ...
        if not files:
            raise RuntimeError("Codex MCP did not return CI files (MCP-only mode)")
        for rel, content in files.items():
            MCP.write_file(self.name, Path(rel), content)
...
'''
    ...
    def _generate_docker_config(self, spec: ProjectSpec):
        ...
        if not files:
            raise RuntimeError("Codex MCP did not return Docker files (MCP-only mode)")
        for rel, content in files.items():
            MCP.write_file(self.name, Path(rel), content)
...
'''
    ...
    def _generate_deployment_config(self, spec: ProjectSpec):
        ...
        if not files:
            raise RuntimeError("Codex MCP did not return deploy files (MCP-only mode)")
        for rel, content in files.items():
            MCP.write_file(self.name, Path(rel), content)
